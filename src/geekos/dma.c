...
/*
 * Only support channels 0..3 for now.  (neil working on through 7)
 */
#define                                             \
  VALID_CHANNEL(chan)	(((chan) >= 0) && ((chan) < 8))
...
#define DMA_MASK_ONE_REG(chan) (chan<4 ? 0x0A : 0xD4)   /*!< Set/reset one bit of mask (output) */
#define DMA_MODE_REG(chan)	(chan<4 ? 0x0B : 0xD6)  /*!< Mode register (output) */
#define DMA_CLEAR_FF_REG(chan)	(chan<4 ? 0x0C : 0xD8)  /*!< Clear byte pointer flip-flop (output) */
...
/* Base and current address (input,output) */
#define DMA_ADDR_REG(chan)	(chan<4 ? (((chan) & 0x3)<<1) : (0xC0 | ((chan) & 0x3) <<2) )

/* Base and current work count (input,output) */
#define DMA_COUNT_REG(chan)	(DMA_ADDR_REG(chan) + (chan<4 ? 1 : 2))
...
void Setup_DMA(enum DMA_Direction direction, int chan, void *addr_,
    ...
    Debug("Setup_DMA(%s,%d,%x,%d)\n",
          direction == DMA_READ ? "DMA_READ" : "DMA_WRITE", chan, addr,
          size);
...
void Mask_DMA(int chan) {
    ...
    Out_Byte(DMA_MASK_ONE_REG(chan), (1 << 2) | (chan & 3));
}
