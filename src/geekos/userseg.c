...
extern struct User_Context *Create_User_Context(ulong_t size)
{
    ...
    context->csSelector = Selector(USER_PRIVILEGE, false, 0); //code segment index = 0
    context->dsSelector = Selector(USER_PRIVILEGE, false, 1); //data segment index = 1
 ...
 */
int Load_User_Program(char *exeFileData, ulong_t exeFileLength __attribute__((unused)),
                      ...
                      struct User_Context **pUserContext)
{
    ...
    ulong_t maxva = 0;
    unsigned numArgs;
    ulong_t argBlockSize;       //argument segment size
     ...
     * to run the process.
     */
    //User process size = total argument block size + process stack size (8192)
