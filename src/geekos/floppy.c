 ...
 * by the CMOS.
 */
static void Setup_Drive_Parameters(int drive, unsigned int type) {
 ...
 */
static void LBA_To_CHS(struct Floppy_Drive *drive, int lba, int *cylinder,
                       int *head, int *sector) {
...
static void Wait_For_MRQ(uchar_t readyValue) {
    ...
    while ((In_Byte(FDC_STATUS_REG) & FDC_STATUS_READY_MASK) !=
           readyValue) ;
...
static void Start_Motor(int drive) {
    Out_Byte(FDC_DOR_REG,
             FDC_DOR_MOTOR(drive) | FDC_DOR_DMA_ENABLE |
             FDC_DOR_RESET_DISABLE | FDC_DOR_DRIVE_SELECT(0));
}

static void Stop_Motor(int drive __attribute__ ((unused))) {
    Out_Byte(FDC_DOR_REG,
             FDC_DOR_DMA_ENABLE | FDC_DOR_RESET_DISABLE |
...
static int Floppy_Transfer(int direction, int driveNum, int blockNum) {
    ...
    LBA_To_CHS(&s_driveTable[driveNum], blockNum, &cylinder, &head,
               &sector);
...
static int Floppy_Read(int driveNum, int blockNum, char *buffer) {
    ...
    rc = Floppy_Transfer(FLOPPY_READ, driveNum, blockNum);
...
static int Floppy_Write(int driveNum, int blockNum, char *buffer) {
    ...
    return Floppy_Transfer(FLOPPY_WRITE, driveNum, blockNum);
}

/*
 * This is the thread that processes floppy I/O requests.
 */
static void Floppy_Request_Thread(ulong_t arg __attribute__ ((unused))) {
    int rc;
...
void Init_Floppy(void) {
    ...
    if(!ready)
        ...
        Print("  Floppy controller initialization complete\n");
}
