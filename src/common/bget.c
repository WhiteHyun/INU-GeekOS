...
#define SizeQuant   4U          /* Buffer allocation size quantum:
...
#define sentinel_check(exp) KASSERT0((exp).sentinel == SENTINEL, "Sentinel check failed: memory corruption, double free, or free of unallocated memory detected")

#elif defined(GEEKOS_LIBC)

#include <string.h>             // for memset()
#include <conio.h>              // for assert()
#define sentinel_check(exp) assert((exp).sentinel == SENTINEL)
...
/* Gmem, little endian */
#define SENTINEL 0x6d656d47

struct bhead {
    unsigned int sentinel;      /* nspring */
...
#define SizeQ	((unsigned int)((SizeQuant > QLSize) ? SizeQuant : QLSize))
...
bufsize requested_size;
{
    ...
    if((unsigned int)size < SizeQ) {    /* Need at least room for the */
    ...
    while (1) {
        ...
        while (b != &freelist) {
            ...
            if((bufsize) b->bh.bsize >= size) {
                ...
                /* nspring: bh.bsize >= size above, so this cast is safe */
                if((unsigned int)(b->bh.bsize - size) >
                   (SizeQ + (sizeof(struct bhead)))) {
                    ...
                    /* nspring: new buffer, set sentinel. */
                    ba->sentinel = SENTINEL;
...
void *buf;
{
    ...
    if(b->bh.prevfree != 0) {
        ...
        b->bh.sentinel = 0;     /* nspring: not a buffer anymore */
...
bufsize len;
{
    ...
    sentinel_check(freelist.ql.blink->bh);
    sentinel_check(freelist.ql.flink->bh);
    ...
    while (b->bh.bsize != ESent) {
        ...
        if(bs < 0) {
            ...
            if((bs > sizeof(struct bfhead)) &&
               ((*lerr != 0x55) ||
                (memcmp(lerr, lerr + 1,
                        (MemSize) (bs - (sizeof(struct bfhead) + 1))) !=
                 0))) {
                V printf
                    ("(Contents of above free block have been overstored.)\n");
                bufdump((void *)(((char *)b) + sizeof(struct bhead)));
